...
def run_satark_metrics(
    ...
    try:
        dataset = SimhasthaDataset(root_dir=data_root, split=split, crop_size=crop_size, downsample=downsample)
    except Exception as exc:
    ...
    accuracy = max(0.0, 100.0 - avg_rel)
    within_10_count = sum(
        1
        for entry in entries
        if (entry['gt'] == 0 and entry['pred'] == 0)
        or (entry['gt'] > 0 and abs(entry['pred'] - entry['gt']) / entry['gt'] <= 0.10)
    )
    within_10 = float(within_10_count / len(entries) * 100)
    ...
    print(f'  Within 10%    : {within_10:.1f}% ({within_10_count}/{len(entries)})')
